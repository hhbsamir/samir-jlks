...
export function PageHeader({ title, children, centerTitle = false }: PageHeaderProps) {
  return (
    <div className={cn(
      ...
      <h1 className={cn(
        "font-headline text-2xl sm:text-3xl font-bold text-primary",
        !centerTitle && "bg-primary/10 border border-primary/20 rounded-lg py-2 px-4 sm:py-3 sm:px-6 shadow-lg"
      )}>
        {title}
      </h1>
